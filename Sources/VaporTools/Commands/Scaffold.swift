...
struct Scaffold: ParsableCommand {
    ...
    func resourceGenerator(name: String, timestamp: String) -> String {
      return """
    import Fluent
    ...
    func run() throws {
      ...
      FileGenerator.createFileWithContents(resource, fileName: "\(name.capitalized)Resource.swift", path: .ResourcePath)
      FileGenerator.createFileWithContents(migration, fileName: "\(timestamp)_\(name.capitalized).swift", path: .MigrationPath)
    }
}
